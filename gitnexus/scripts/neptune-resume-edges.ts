/**
 * Resume script: creates indexes then inserts only edges into Neptune.
 * Use when nodes are already loaded but edges failed/timed out.
 *
 * Usage:
 *   GITNEXUS_COBOL_DIRS=s,c,wfproc node --max-old-space-size=8192 \
 *     --loader ts-node/esm scripts/neptune-resume-edges.ts <repo-path>
 */
...
async function sendCypher(client: NeptunedataClient, cypher: string, params?: Record<string, unknown>): Promise<void> {
...
async function main() {
  ...
  // 1. Run pipeline to get graph in memory
  console.log('  [1/3] Running pipeline to extract graph...');
  const t0 = Date.now();
  ...
  console.log(`\n\n  Done! ${inserted.toLocaleString()} edges inserted in ${((Date.now() - t1) / 1000).toFixed(0)}s`);
  client.destroy();
  process.exit(0);
}
