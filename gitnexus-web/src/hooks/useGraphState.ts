...
export function useGraphState(switchRepoDeps?: SwitchRepoDeps): GraphState {
  ...
  const switchRepo = useCallback(async (repoName: string) => {
    ...
    const { setProgress, setHighlightedNodeIds, clearAIToolHighlights, clearBlastRadius, setQueryResult, initializeAgent, startEmbeddings, hydrateWorkerFromServer } = depsRef.current;
